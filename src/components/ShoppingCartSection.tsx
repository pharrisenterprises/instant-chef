...
/**
 * Fixes:
 * - adds missing `useState` import (used by AddExtraItem)
...
export default function ShoppingCart({
  ...
  const totalMeal = useMemo(
    () => cartMeal.reduce((a, c) => a + c.estPrice, 0),
    [cartMeal]
  );
  const totalExtra = useMemo(
    () => cartExtra.reduce((a, c) => a + c.estPrice, 0),
    [cartExtra]
  );
  const grandTotal = useMemo(
    () => +(totalMeal + totalExtra).toFixed(2),
    [totalMeal, totalExtra]
  );
    ...
    <div className="bg-white rounded-2xl shadow p-6">
      ...
      <div className="space-y-4">
        <div className="rounded-lg border p-4 bg-gray-50">
          <p className="text-sm">
            Meal Ingredients Subtotal:{" "}
            <span className="font-semibold">${totalMeal.toFixed(2)}</span>
          </p>
          <p className="text-sm">
            Additional Items Subtotal:{" "}
            <span className="font-semibold">${totalExtra.toFixed(2)}</span>
          </p>
          <p className="text-lg">
...
function AddExtraItem({
  ...
  onAdd: (name: string, qty: number, measure: Measure, price: number) => void;
}) {
