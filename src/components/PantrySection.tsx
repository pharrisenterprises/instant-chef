...
export default function PantrySection({
  ...
  pantry: PantryItem[]
  addPantryManual: (name: string, qty: number | null, measure: Measure | null, type?: string) => void
              ...
              {editingPantryItem === item.id ? (
                ...
                <>
                  ...
                  <div className="flex items-center gap-2">
                    <button className="text-xs px-2 py-1 border rounded" onClick={() => setEditingPantryItem(item.id)} title="Edit">
                      ✏️
                    </button>
                    <button className="text-xs px-2 py-1 border rounded" onClick={() => setEditForm(p => ({ ...p, pantry: pantry.filter(p => p.id !== item.id) }))}>
                      Remove
                    </button>
                  </div>
                </>
